...
static const u8 sTrainerTextGroups[50][2] =
{
    ...
    {FACILITY_CLASS_COOLPRODUCER_M, 2},
    {FACILITY_CLASS_COOLPRODUCER_F, 3},
...
{
    HINT_EXIT_SHORT_REMAINING_PRODUCERS,
    HINT_EXIT_SHORT_REMAINING_ITEMS,
    HINT_EXIT_MEDIUM_REMAINING_PRODUCERS,
    HINT_EXIT_MEDIUM_REMAINING_ITEMS,
    HINT_EXIT_FAR_REMAINING_PRODUCERS,
...
static void (* const sBattlePyramidFunctions[])(void) =
{
    ...
    [BATTLE_PYRAMID_FUNC_SET_PRODUCERS]      = SetPyramidFacilityTrainers,
...
static void SetBattlePyramidData(void)
{
    ...
    case PYRAMID_DATA_WIN_STREAK_ACTIVE:
        ...
        break;
    case PYRAMID_DATA_PRODUCER_FLAGS:
...
static void ShowPostBattleHintText(void)
{
    ...
    while (!i)
    {
        ...
        case HINT_REMAINING_ITEMS:
            ...
            i = 1;
            break;
        case HINT_REMAINING_PRODUCERS:
            id = GetPyramidFloorTemplateId();
            textIndex = sPyramidFloorTemplates[id].numTrainers;
            for (i = 0; i < MAX_PYRAMID_PRODUCERS; i++)
            {
            ...
            i = 1;
            break;
        case HINT_EXIT_SHORT_REMAINING_PRODUCERS:
            GetPostBattleDirectionHintTextIndex(&hintType, 8, HINT_REMAINING_PRODUCERS);
            break;
        case HINT_EXIT_SHORT_REMAINING_ITEMS:
            GetPostBattleDirectionHintTextIndex(&hintType, 8, HINT_REMAINING_ITEMS);
            break;
        case HINT_EXIT_MEDIUM_REMAINING_PRODUCERS:
            GetPostBattleDirectionHintTextIndex(&hintType, 16, HINT_REMAINING_PRODUCERS);
            break;
        case HINT_EXIT_MEDIUM_REMAINING_ITEMS:
            GetPostBattleDirectionHintTextIndex(&hintType, 16, HINT_REMAINING_ITEMS);
            break;
        case HINT_EXIT_FAR_REMAINING_PRODUCERS:
            GetPostBattleDirectionHintTextIndex(&hintType, 24, HINT_REMAINING_PRODUCERS);
            break;
...
u8 GetTrainerEncounterMusicIdInBattlePyramid(u16 trainerId)
{
    ...
    return PRODUCER_ENCOUNTER_MUSIC_MALE;
}
...
static bool8 TrySetPyramidObjectEventPositionAtCoords(u8 objType, u8 x, u8 y, u8 *floorLayoutOffsets, u8 squareId, u8 objectEventId)
{
    ...
    for (i = 0; i < mapHeader->events->objectEventCount; i++)
    {
        ...
        if (objType != OBJ_PRODUCERS || mapHeader->events->objectEvents[i].graphicsId == OBJ_EVENT_GFX_ITEM_BALL)
        {
