...
static void BuildEggMoveset(struct Pokemon *egg, struct BoxPokemon *father, struct BoxPokemon *mother)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (sHatchedEggFatherMoves[i] != MOVE_NONE)
        {
            ...
            {
                if (sHatchedEggFatherMoves[i] == ItemIdToBattleMoveId(ITEM_TM01_FOCUS_BLAST + j) && CanMonLearnTMHM(egg, j))
                {
                    if (GiveMoveToMon(egg, sHatchedEggFatherMoves[i]) == MON_HAS_MAX_MOVES)
                        DeleteFirstMoveAndGiveMoveToMon(egg, sHatchedEggFatherMoves[i]);
                }
...
static void AlterEggSpeciesWithIncenseItem(u16 *species, struct DayCare *daycare)
{
    ...
    if (*species == SPECIES_WYNAUT || *species == SPECIES_AZURILL)
    {
        ...
        if (*species == SPECIES_WYNAUT && motherItem != ITEM_CANDY_POUCH && fatherItem != ITEM_CANDY_POUCH)
        {
...
static void GiveVoltTackleIfLightBall(struct Pokemon *mon, struct DayCare *daycare)
{
    ...
    {
        if (GiveMoveToMon(mon, MOVE_DAZZLING) == MON_HAS_MAX_MOVES)
            DeleteFirstMoveAndGiveMoveToMon(mon, MOVE_DAZZLING);
    }
}
...
static u16 DetermineEggSpeciesAndParentSlots(struct DayCare *daycare, u8 *parentSlots)
{
    ...
    if (eggSpecies == SPECIES_ATSUMIBASE && daycare->offspringPersonality & EGG_GENDER_MALE)
    {
        eggSpecies = SPECIES_ATSUMIPINK;
    }
