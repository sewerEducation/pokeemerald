...
enum {
    ...
    FIELD_MOVE_SPACE_TRAVEL,// FLAG_BADGE06_GET
...
{
    if (GetMonData(mon, MON_DATA_IS_EGG) != TRUE && GetMonData(mon, MON_DATA_SPECIES) == SPECIES_HARUBASE)
        return TRUE;
    return FALSE;
}
...
static void DisplayPartyPokemonGender(u8 gender, u16 species, u8 *nickname, struct PartyMenuBox *menuBox)
{
    ...
    if (species == SPECIES_NONE)
        return;
    if ((species == SPECIES_ATSUMIPINK || species == SPECIES_ATSUMIBASE) && StringCompare(nickname, gSpeciesNames[species]) == 0)
        return;
    switch (gender)
    {
    case MON_MALE:
