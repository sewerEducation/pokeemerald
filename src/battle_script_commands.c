...
static const u8 sTerrainToType[] =
{
    ...
    [BATTLE_TERRAIN_SAND]       = TYPE_ROCK,
    [BATTLE_TERRAIN_UNDERWATER] = TYPE_OCEAN,
    [BATTLE_TERRAIN_WATER]      = TYPE_OCEAN,
    [BATTLE_TERRAIN_POND]       = TYPE_OCEAN,
    [BATTLE_TERRAIN_MOUNTAIN]   = TYPE_WILD,
    [BATTLE_TERRAIN_CAVE]       = TYPE_WILD,
...
static void Cmd_attackcanceler(void)
{
    ...
    if (gSpecialStatuses[gBattlerTarget].lightningRodRedirected)
    {
        ...
        gLastUsedAbility = ABILITY_UNHOLY_VOID;
        BattleScriptPushCursor();
...
static void Cmd_accuracycheck(void)
{
    ...
    else
    {
        ...
        if (gBattleMons[gBattlerAttacker].ability == ABILITY_GLASSES)
...
static void Cmd_critcalc(void)
{
    ...
    critChance  = 2 * ((gBattleMons[gBattlerAttacker].status2 & STATUS2_FOCUS_ENERGY) != 0)
                ...
                + 2 * (holdEffect == HOLD_EFFECT_STICK && gBattleMons[gBattlerAttacker].species == SPECIES_SANAGOLD);

    if (critChance >= ARRAY_COUNT(sCriticalHitChance))
        critChance = ARRAY_COUNT(sCriticalHitChance) - 1;

    if ((gBattleMons[gBattlerTarget].ability != ABILITY_HERO_ARMOR && gBattleMons[gBattlerTarget].ability != ABILITY_SHELL_ARMOR)
     && !(gStatuses3[gBattlerAttacker] & STATUS3_CANT_SCORE_A_CRIT)
     && !(gBattleTypeFlags & (BATTLE_TYPE_WALLY_TUTORIAL | BATTLE_TYPE_FIRST_BATTLE))
     && ((!(Random() % sCriticalHitChance[critChance])) || (gBattleMons[gBattlerAttacker].ability == ABILITY_MERCILESS && gBattleMons[gBattlerTarget].status1)))
     {
...
u8 AI_TypeCalc(u16 move, u16 targetSpecies, u8 targetAbility)
{
    ...
    if (targetAbility == ABILITY_LEVITATE && moveType == TYPE_ROCK)
    {
        flags = MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE;
    }
    else
...
void SetMoveEffect(bool8 primary, u8 certain)
{
    ...
    if (gBattleMons[gEffectBattler].ability == ABILITY_BIG_PICTURE && !(gHitMarker & HITMARKER_IGNORE_SAFEGUARD)
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= PRIMARY_STATUS_MOVE_EFFECT)
    {
        ...
        case STATUS1_POISON:
            ...
            if ((IS_BATTLER_OF_TYPE(gEffectBattler, TYPE_TOXIC) || IS_BATTLER_OF_TYPE(gEffectBattler, TYPE_METAL))
                && (gHitMarker & HITMARKER_IGNORE_SAFEGUARD)
                && (primary == TRUE || certain == MOVE_EFFECT_CERTAIN))
            {
                BattleScriptPush(gBattlescriptCurrInstr + 1);
            ...
            if (IS_BATTLER_OF_TYPE(gEffectBattler, TYPE_TOXIC))
                break;
        ...
        case STATUS1_FREEZE:
            ...
            if (IS_BATTLER_OF_TYPE(gEffectBattler, TYPE_COLD))
                break;
            if (gBattleMons[gEffectBattler].status1)
                break;
            if (noSunCanFreeze == FALSE)
                break;
            if (gBattleMons[gEffectBattler].ability == ABILITY_HAUGHTINESS)
                break;
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_ON_DAMAGE_ABILITIES: // Such as abilities activating on contact (Effect Spore, Rough Skin, etc.).
            ...
            gBattleScripting.moveendState++;
            break;
        case MOVEEND_ON_DAMAGE_ABILITIES_ATTACKER: // attacker abilities activating on contact stench,poison touch etc
...
static void Cmd_switchineffects(void)
{
    ...
    {
        // There is a hack here to ensure the nervous counter will be 0 when the battler's next turn starts.
        // The nervous counter is not updated in the case where a mon switches in after a lost judgement in the battle arena.
        if (gBattleMons[gActiveBattler].ability == ABILITY_NERVOUS && !gDisableStructs[gActiveBattler].nervousSwitchInHack)
            gDisableStructs[gActiveBattler].nervousCounter = 1;

        gDisableStructs[gActiveBattler].nervousSwitchInHack = 0;
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_ARENA_OPPONENT_MON_LOST:
        ...
        gDisableStructs[1].nervousSwitchInHack = 1;
        break;
    case VARIOUS_ARENA_PLAYER_MON_LOST:
        ...
        gDisableStructs[0].nervousSwitchInHack = 1;
        break;
...
static void Cmd_tryexplosion(void)
{
    ...
    // Explosion can only fail if any battler has Carefree Air
    for (gBattlerTarget = 0; gBattlerTarget < gBattlersCount; gBattlerTarget++)
    {
        if (gBattleMons[gBattlerTarget].ability == ABILITY_CALM_AURA)
            break;
    }

    if (gBattlerTarget == gBattlersCount)
    {
        // Success, no battlers with Carefree Air. Drop user's HP bar to 0
        gActiveBattler = gBattlerAttacker;
    ...
    {
        // Failed, a battler has Carefree Air
        gLastUsedAbility = ABILITY_CALM_AURA;
        RecordAbilityBattle(gBattlerTarget, gBattleMons[gBattlerTarget].ability);
        gBattlescriptCurrInstr = BattleScript_CalmAuraStopsExplosion;
    }
}
...
static u8 ChangeStatBuffs(s8 statValue, u8 statId, u8 flags, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        else if (gBattleMons[gActiveBattler].ability == ABILITY_BIG_PICTURE && flags == 0)
        {
            return STAT_CHANGE_DIDNT_WORK;
        }
...
static void Cmd_weatherdamage(void)
{
    if (WEATHER_HAS_EFFECT)
    {
        if (gBattleWeather & B_WEATHER_SANDSTORM)
        {
            if (gBattleMons[gBattlerAttacker].type1 != TYPE_WILD
                && gBattleMons[gBattlerAttacker].type1 != TYPE_METAL
                && gBattleMons[gBattlerAttacker].type1 != TYPE_ROCK
                && gBattleMons[gBattlerAttacker].type2 != TYPE_WILD
                && gBattleMons[gBattlerAttacker].type2 != TYPE_METAL
                && gBattleMons[gBattlerAttacker].type2 != TYPE_ROCK
                && gBattleMons[gBattlerAttacker].ability != ABILITY_SAND_VEIL
                && gBattleMons[gBattlerAttacker].ability != ABILITY_WEATHERPROOF
        ...
        {
            if (!IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_COLD)
...
static void Cmd_jumpifnopursuitswitchdmg(void)
{
    ...
    if (gChosenActionByBattler[gBattlerTarget] == B_ACTION_USE_MOVE
        ...
        && !gDisableStructs[gBattlerTarget].nervousCounter
...
static void Cmd_setweatherballtype(void)
{
    if (WEATHER_HAS_EFFECT)
    {
        ...
        if (gBattleWeather & B_WEATHER_RAIN)
            ...
            *(&gBattleStruct->dynamicMoveType) = TYPE_WILD | F_DYNAMIC_TYPE_2;
        else if (gBattleWeather & B_WEATHER_SUN)
            *(&gBattleStruct->dynamicMoveType) = TYPE_HOT | F_DYNAMIC_TYPE_2;
        else if (gBattleWeather & B_WEATHER_HAIL)
            *(&gBattleStruct->dynamicMoveType) = TYPE_COLD | F_DYNAMIC_TYPE_2;
        else
